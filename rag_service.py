...
def missing_context_answer() -> str:
    return (
        "Mình chưa tìm thấy thông tin đủ rõ trong tài liệu để trả lời câu hỏi này."
    )
...
retriever = vector_store.as_retriever(
    ...
    run_name="qdrant_retriever",
    tags=["rag", "retrieval", "qdrant", "api"],
    metadata={"collection_name": COLLECTION_NAME, "top_k": top_k},
)
...
rewrite_prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """Bạn tối ưu câu hỏi tiếng Việt để tìm kiếm trong kho tài liệu.
...
rewrite_chain = (rewrite_prompt | llm | StrOutputParser()).with_config(
    run_name="rewrite_question",
    tags=["rag", "rewrite", "langgraph"],
)
...
grade_prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """Bạn kiểm tra CONTEXT có đủ thông tin để trả lời QUESTION không.
...
grade_chain = (grade_prompt | llm | StrOutputParser()).with_config(
    run_name="grade_context",
    tags=["rag", "grade", "langgraph"],
)
...
answer_chain = (answer_prompt | llm | StrOutputParser()).with_config(
    run_name="generate_answer",
    tags=["rag", "answer", "langgraph"],
)
...
rag_graph = workflow.compile().with_config(
    run_name="rag_qdrant_graph",
    tags=["rag", "qdrant", "langgraph", "api"],
    metadata={"collection_name": COLLECTION_NAME},
)

def run_rag(question: str, config: RunnableConfig | None = None) -> str:
    state = rag_graph.invoke({"question": question}, config=config)
    return state.get("answer", missing_context_answer())


rag_chain = RunnableLambda(run_rag).with_config(
    run_name="rag_qdrant_api",
    tags=["rag", "qdrant", "langserve", "langgraph", "api"],
    metadata={"collection_name": COLLECTION_NAME},
)
